...
<aside class="w-64 bg-[#4C9A8B] text-white flex flex-col min-h-screen">
    ...
    <!-- ================= CABANG / KOTA ================= -->
    @if(in_array($role, ['admin','finance','superadmin']))
    ...
    <nav class="flex-1 px-3 py-4 space-y-1 text-sm">
        ...
        @elseif($role === 'finance')
            ...
            <div x-data="{ open: true }">
                ...
                <div x-show="open" x-transition
                    ...
                    <a href="{{ route('finance.transaction.transfer') }}" class="block text-sm hover:underline">
                        Pembayaran Transfer
                    </a>

                    <a href="{{ route('finance.transaction.cash') }}" class="block text-sm hover:underline">
                        Pembayaran Cash
                    </a>

                    <a href="{{ route('finance.transaction.cancelled') }}" class="block text-sm hover:underline">
                        Dibatalkan
                    </a>

                    <a href="{{ route('finance.transaction.reschedule') }}" class="block text-sm hover:underline">
                        Reschedule
                    </a>
        ...
        @elseif($role === 'superadmin')
            ...
            {{-- FULL dari file kamu --}}
            {{-- referensi asli: :contentReference[oaicite:0]{index=0} --}}

            <a href="{{ route('superadmin.dashboard') }}"
               class="flex items-center gap-3 px-4 py-3 rounded-lg
               {{ request()->routeIs('superadmin.dashboard') ? 'bg-white text-[#3E7F73]' : 'hover:bg-white/10' }}">
                Dashboard
            </a>
